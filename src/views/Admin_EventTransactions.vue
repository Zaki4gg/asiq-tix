...
async function loadPurchases () {
  ...
  errorMsg.value = ''
  try {
    const res = await api(`/api/admin/events/${encodeURIComponent(eventId.value)}/transactions`)
    ...
    <main class="container dashboard">
      <section class="hero hero--compact">
        <div class="hero-text">
          ...
          <p v-if="eventInfo?.promoter_wallet" class="muted">
            Promoter: <span class="mono">{{ eventInfo.promoter_wallet }}</span>
          </p>
        </div>
...
.hero-text .muted{ color: var(--muted); font-size: 14px; margin: 4px 0 0; }
...
.btn:hover{ background: rgba(244,241,222,.24); }
.btn.secondary{ background: rgba(244,241,222,.18); }
