...
// Hardening: kalau router context tidak tersedia, jangan crash.
let route = null
try {
  route = useRoute()
} catch {
...
/**
 * Sembunyikan footer di:
 * - login: "/"  (umumnya login Anda)
 * - scan:  "/scan"
 * - fallback: "/login" dan name "login" kalau Anda pakai itu
 */
const hidden = computed(() => {
  const p = route?.path ?? ''
  const n = route?.name ?? ''
  return p === '/' || p === '/scan' || p === '/login' || n === 'login'
})
...
<template>
  <footer v-if="!hidden" class="site-footer" role="contentinfo">
    <div class="footer-inner">
      ...
      <section v-for="(o, i) in offices" :key="i" class="office-cell">
